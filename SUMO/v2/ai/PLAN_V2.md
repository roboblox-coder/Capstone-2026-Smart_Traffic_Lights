...
4. `ai/sumo_calibration/` — Phase 1.1 (**scaffold built**). Three
   modules:
   - `build_calibrated_routes.py`: refactors `real_life_simulation.py`
     into a library with a sidecar `provenance.json` tagging
     `real | estimated` per intersection and per edge. Non-interactive
     (the legacy `input()` fallback is replaced with a hard error).
   - `calibrate_carfollow.py`: emits `vTypes_calibrated.add.xml` with
     Krauss params pinned to HCM 6 / SUMO-defaults and per-field
     provenance. Optional grid-search calibration mode is stubbed
     pending saturation-flow surveys (see `report.md` R3).
   - `report.md`: what's real vs estimated, plus the priority-ordered
     city-engagement TODO list.
