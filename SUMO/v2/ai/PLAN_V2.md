...
- Pin the V1 deliverable: `ai/runs/coordinated/checkpoints/<tls>/best.pth`
  + `eval_network.txt` numbers above + the regime constants.
- Add a **CI-style regression test** that re-runs `ai/eval_network.py`
  on 3 fixed seeds and fails if either headline metric regresses by
  more than 2σ from the locked baseline. Gate every future change on it.
- Snapshot `sim.sumocfg` + `harvard_simulation.rou.xml` as the v1.0
  simulation environment. Any change to demand / routes bumps the env
  version and forces a re-lock.

**Gate:** regression test runs green on a clean clone.
...
2. `ai/regression_test.py` — Phase 0. Runs `eval_network.py` on 3 seeds,
   compares to a checked-in `ai/baseline_v1.json`, exits non-zero on
   regression > 2σ.
3. `ai/baseline_v1.json` — Phase 0. Pin the V1 headline numbers.
